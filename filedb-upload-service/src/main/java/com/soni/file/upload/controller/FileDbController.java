...
import com.soni.file.upload.service.DbStorageService;
...
//        return dbStorageService.save(filePartMono).map((filename)
...
//        dbStorageService.saveAll(filePartFlux);
//        return dbStorageService.saveAll(filePartFlux).collectList().flatMap(filenames -> {
...
//        });
    ...
    public ResponseEntity<Flux<FileMetaInfo>> readAllFiles() throws IOException {
        return ResponseEntity.status(HttpStatus.OK).body(dbStorageService.loadAll());
        return null;
    }
